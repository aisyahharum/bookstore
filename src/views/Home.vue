  ...
  <div>
    ...
    <v-container grid-list-md>
      <v-subheader>
        Random Category
        <v-spacer></v-spacer>
        <!-- link ke route categories yang nantinya akan didefinisikan routing dan componentnya -->
        <router-link to="/categories">See All</router-link>
      </v-subheader>
      <v-layout row wrap>
        <!-- var categories ini nantinya akan kita isi dengan data dari endpoint category  -->
        <v-flex v-for="category in categories" xs6 :key="category.id">
          <v-card :to="'/category/'+ category.slug">
            <!-- untuk load img supaya lebih rapi akan kita buatkan method getimg -->
